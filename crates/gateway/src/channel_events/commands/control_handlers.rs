...
pub(in crate::channel_events) async fn handle_model(
    ...
    let current_choice = if let Some(kind) = current_external_agent {
        Some(external_agent_model_id(kind.as_str()))
    } else {
        current_model
    };
    ...
    if args.is_empty() {
        ...
        if let Some(kind) = chosen
            ...
            .and_then(|v| v.as_str())
            .or_else(|| model_id.strip_prefix(EXTERNAL_AGENT_MODEL_PREFIX))
        {
            let kind = kind
    ...
    Ok(agents
        ...
        .filter_map(|agent| {
            ...
            Some(serde_json::json!({
                "id": external_agent_model_id(kind),
                "provider": EXTERNAL_AGENT_PROVIDER,
                "displayName": external_agent_display_name(kind),
...
fn external_agent_model_id(kind: &str) -> String {
    format!("{EXTERNAL_AGENT_MODEL_PREFIX}{kind}")
}
